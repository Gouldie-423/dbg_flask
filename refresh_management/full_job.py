import csv
import refresh_management.db_refresh as refresh
import datetime as dt
from datetime import timedelta
from datetime import datetime
import refresh_management.load_data as load
import refresh_management.basic_rnn_model as rnn
import refresh_management.screening_alg as alg
import numpy as np
...
def dupe_check(data,last_data_received,today):
	...
	try:
		...
		last_data_received = datetime.strptime(last_data_received, '%Y-%m-%dT00:00:00.000000000').date()
		...
		return []
	except IndexError: #system returns index error when no new data is received: index -1 is out of bounds for axis 0 with size 0.
		return []		#most likely happening due to weekend refreshes. During week up to date data shouldn't be in missing_data table due to def missing_data_check conditional logic 
...
def backfill_trade_data(): #need to remove arguments to import as cron job to flask
	...
	#making avtive s&p 500 tickers are aligned with db
	#no longer need to return working tickers b/c refresh is based on missing data
	working_tickers = ticker_check(today,engine) 

	#deleting missing data before each refresh
	load.delete_missing_data(engine)
	#refreshing missing data
	max_dates = load.query_recent_date(engine)
	missing_data_refresh(working_tickers,today,max_dates,engine)
	# could probably return missing_tickers from above function
	...
	buy_search(working_tickers,today,engine)
	sell_search(today,engine)

	#deleting missing data after new data has added to db
	load.delete_missing_data(engine)
	...
	#don't have a ton of concern surrounding missing_data table deleting data.
	#table refreshes every 30 minutes. Given the number of missing tickers on a day to day basis
	#better to delete them to save space over time
