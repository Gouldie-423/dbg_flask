...
class model:
    ...
    def load_params(self):
        ...
        for i in range(0,len(self.layers_dims['rnn'])):
            self.params['Waa'+str(i+1)] = torch.load(f'./refresh_management/V{self.algorithm}/Waa{str(i+1)}.pt')
            self.params['Wax'+str(i+1)] = torch.load(f'./refresh_management/V{self.algorithm}/Wax{str(i+1)}.pt')
            self.params['ba'+str(i+1)] = torch.load(f'./refresh_management/V{self.algorithm}/ba{str(i+1)}.pt')

        for i in range(0,len(self.layers_dims['ff'])-1):
            self.params['W'+str(i+1)] = torch.load(f'./refresh_management/V{self.algorithm}/W{str(i+1)}.pt')
            self.params['b'+str(i+1)] = torch.load(f'./refresh_management/V{self.algorithm}/b{str(i+1)}.pt')
